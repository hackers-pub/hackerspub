...
test("Account.unreadNotificationsCount counts notifications newer than notificationRead", async () => {
...
test("Account.unreadNotificationsCount counts all notifications when never read", async () => {
  await withRollback(async (tx) => {
    const recipient = await insertAccountWithActor(tx, {
...
test("markNotificationsAsRead respects the optional upTo notification", async () => {
  await withRollback(async (tx) => {
    const recipient = await insertAccountWithActor(tx, {
...
test("markNotificationsAsRead preserves database timestamp precision", async () => {
  await withRollback(async (tx) => {
    const recipient = await insertAccountWithActor(tx, {
...
test("Notification.actors batches across multiple notifications", async () => {
  await withRollback(async (tx) => {
    const recipient = await insertAccountWithActor(tx, {
...
test("Notification.actors filters out missing actor ids without breaking the batch", async () => {
  await withRollback(async (tx) => {
    const recipient = await insertAccountWithActor(tx, {
...
test("Notification.actors fires one DataLoader batch for the deduped actor id union", async () => {
  await withRollback(async (tx) => {
    const recipient = await insertAccountWithActor(tx, {
    ...
    assert.deepEqual(
      new Set(batches[0]),
      new Set([actorA.actor.id, actorB.actor.id, actorC.actor.id]),
    );
  });
});

test("Notification.actors deduplicates repeated actor ids within a notification", async () => {
  await withRollback(async (tx) => {
    const recipient = await insertAccountWithActor(tx, {
