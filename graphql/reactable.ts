  ...
  fields: (t) => ({
    reactionGroups: t.field({
      ...
      async resolve(post, _args, ctx) {
        const groups = Object.entries(post.reactionsCounts).map(
          ([emojiOrId, count]):
            | EmojiReactionGroup
...
export const ReactionGroup = builder
  .interfaceRef<ReactionGroup>("ReactionGroup")
  .implement({
    resolveType(group) {
      switch (group.type) {
        case "Emoji":
            ...
            viewerHasReacted: t.loadable({
              ...
              load: async (
                ...
                const rowsByViewer = new Map<
                  Uuid,
                  Awaited<ReturnType<typeof getViewerReactionsForPosts>>
                >();
                ...
                for (const [viewerActorId, postIds] of postIdsByViewer) {
                  rowsByViewer.set(
                    viewerActorId,
                    await getViewerReactionsForPosts(ctx.db, [...postIds], {
                      id: viewerActorId,
                    }),
                  );
                }
                return decoded.map(
                  (key) =>
                    ...
                    (rowsByViewer.get(key.viewerActorId) ?? []).some(
                      (row) =>
...
const EmojiReactionGroup =
  builder.objectRef<EmojiReactionGroup>("EmojiReactionGroup");
