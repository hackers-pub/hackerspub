...
test("invite validates verify URLs that do not interpolate the token", async () => {
...
test("invite sends email and stores a signup token on success", async () => {
  await withRollback(async (tx) => {
    ...
    assert.deepEqual(invitation.message, "Join us");
    assert.deepEqual(invitation.inviter?.username, "inviteowner");
    ...
    assert.deepEqual(token.email, "invitee@example.com");
    assert.deepEqual(token.inviterId, inviter.account.id);
  });
});

test("invite refunds invitations when email sending fails", async () => {
  await withRollback(async (tx) => {
    const { kv, store } = createTestKv();
    const inviter = await insertAccountWithActor(tx, {
    ...
    assert.deepEqual(storedAccount?.leftInvitations, 1);
    const signupEntries = [...store.entries()];
    assert.deepEqual(signupEntries.length, 0);
  });
});
