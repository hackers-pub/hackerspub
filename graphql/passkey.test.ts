...
test("getPasskeyRegistrationOptions stores a challenge for the signed-in account", async () => {
...
test("getPasskeyAuthenticationOptions and loginByPasskey return null for unknown credentials", async () => {
  await withRollback(async (tx) => {
    const { kv, store } = createTestKv();
...
test("revokePasskey deletes an existing passkey and returns its global ID", async () => {
  await withRollback(async (tx) => {
