...
test("sendNotificationDigests sends one daily digest across unread notification sources", async () => {
...
test("sendNotificationDigests retries failed digest deliveries", async () => {
  await withRollback(async (tx) => {
    ...
    const failedDeliveries = await tx
      .select()
      .from(notificationDigestDeliveryTable)
      .where(eq(notificationDigestDeliveryTable.accountId, account.account.id));
...
test("sendNotificationDigests does not resend to successful recipients after partial failures", async () => {
  await withRollback(async (tx) => {
    const account = await insertAccountWithActor(tx, {
    ...
    const partialEmail = {
      async send(message: Message) {
        ...
        return {
          ...
          errorMessages:
            address === "digestretrypartial@example.com"
              ? []
    ...
    assert.deepEqual(attempted, ["digestretrypartial-backup@example.com"]);
    const sentDeliveries = await tx
      .select()
      .from(notificationDigestDeliveryTable)
      .where(eq(notificationDigestDeliveryTable.accountId, account.account.id));
...
test("weekly digest suppresses daily digest for weekly-enabled accounts on Mondays", async () => {
  await withRollback(async (tx) => {
    const account = await insertAccountWithActor(tx, {
