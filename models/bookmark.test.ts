...
test("arePostsBookmarkedBy() returns the subset of posts the account bookmarked", async () => {
...
test("arePostsBookmarkedBy() returns an empty set when no bookmarks match", async () => {
  await withRollback(async (tx) => {
    const author = await insertAccountWithActor(tx, {
    ...
    const result = await arePostsBookmarkedBy(tx, [post.id], viewer.account);

    assert.deepEqual(result, new Set());
  });
});

test("arePostsBookmarkedBy() returns an empty set for an empty input list", async () => {
  await withRollback(async (tx) => {
    const viewer = await insertAccountWithActor(tx, {
