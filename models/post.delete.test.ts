...
test("deletePost() removes a reply and decrements the parent reply count", async () => {
  await withRollback(async (tx) => {
    ...
    assert.ok(storedRoot != null);
    assert.deepEqual(storedRoot.repliesCount, 0);

    const storedReply = await tx.query.postTable.findFirst({
      where: { id: replyPost.id },
    });
    assert.deepEqual(storedReply, undefined);

    const replySource = await tx.query.noteSourceTable.findFirst({
      where: { id: replySourceId },
    });
    assert.deepEqual(replySource, undefined);
  });
});

test("deletePost() deletes a standalone post without querying unrelated originals", async () => {
  await withRollback(async (tx) => {
    const fedCtx = createFedCtx(tx);
    const author = await insertAccountWithActor(tx, {
    ...
    assert.deepEqual(deletedSource, undefined);

    const unrelatedPost = await tx.query.postTable.findFirst({
      where: { id: unrelated.id },
    });
    assert.ok(unrelatedPost != null);
    assert.deepEqual(unrelatedPost.id, unrelated.id);
  });
});

test("deletePost() cascades through local replies, shares, and notifications", async () => {
  await withRollback(async (tx) => {
    const fedCtx = createFedCtx(tx);
    const author = await insertAccountWithActor(tx, {
    ...
    const { noteSourceId: rootSourceId, post: rootPost } = await insertNotePost(
      tx,
      {
        account: author.account,
