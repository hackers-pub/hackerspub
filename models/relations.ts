...
export const relations = defineRelations(schema, (r) => ({
  ...
  deletedAccountKeyTable: {
    deletedAccount: r.one.deletedAccountTable({
      from: r.deletedAccountKeyTable.accountId,
      to: r.deletedAccountTable.accountId,
    }),
  },
  accountLinkTable: {
    account: r.one.accountTable({
  ...
  quoteAuthorizationTable: {
    quotePost: r.one.postTable({
      from: r.quoteAuthorizationTable.quotePostId,
      to: r.postTable.id,
    }),
    quotedPost: r.one.postTable({
      from: r.quoteAuthorizationTable.quotedPostId,
      to: r.postTable.id,
      optional: false,
      alias: "quotedPost",
    }),
    attributedActor: r.one.actorTable({
      from: r.quoteAuthorizationTable.attributedActorId,
      to: r.actorTable.id,
      optional: false,
    }),
  },
