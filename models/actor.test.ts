...
test("getPersistedActor() loads local actor with account and instance", async () => {
  await withRollback(async (tx) => {
    ...
    assert.deepEqual(actor.id, account.actor.id);
    assert.deepEqual(actor.account?.id, account.account.id);
    assert.deepEqual(actor.instance.host, "localhost");
    assert.deepEqual(actor.successor, null);
  });
});

test("getActorStats() counts notes, replies, shares, and articles", async () => {
  await withRollback(async (tx) => {
