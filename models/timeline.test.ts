...
test("expandLocales() adds base language when region-specific locale is given", () => {
...
test("addPostToTimeline() delivers direct posts only to the author and mentions", async () => {
...
test("removePostsFromTimeline() removes shares across batches", async () => {
  await withRollback(async (tx) => {
    ...
    const remaining = await tx
      .select()
      .from(timelineItemTable)
      .where(eq(timelineItemTable.accountId, viewer.account.id));
    assert.equal(remaining.length, 0);
  });
});

test("removeFromTimeline() preserves a surviving share delivered by mention", async () => {
  await withRollback(async (tx) => {
    const fedCtx = createFedCtx(tx);
    const suffix = crypto.randomUUID().replaceAll("-", "").slice(0, 8);
    const viewer = await insertAccountWithActor(tx, {
...
test("removeFromTimeline() preserves a surviving share delivered by quote", async () => {
  await withRollback(async (tx) => {
    const fedCtx = createFedCtx(tx);
    const suffix = crypto.randomUUID().replaceAll("-", "").slice(0, 8);
    const viewer = await insertAccountWithActor(tx, {
...
test("removeFromTimeline() excludes a share that has not been deleted yet", async () => {
  await withRollback(async (tx) => {
    const fedCtx = createFedCtx(tx);
    const suffix = crypto.randomUUID().replaceAll("-", "").slice(0, 8);
    const viewer = await insertAccountWithActor(tx, {
...
test("removeFromTimeline() does not fall back to a sharer the viewer has muted", async () => {
  await withRollback(async (tx) => {
    const fedCtx = createFedCtx(tx);
    const suffix = crypto.randomUUID().replaceAll("-", "").slice(0, 8);
    const viewer = await insertAccountWithActor(tx, {
...
test("removeFromTimeline() does not fall back to a sharer whose boost is not visible to the viewer", async () => {
  await withRollback(async (tx) => {
    const fedCtx = createFedCtx(tx);
    const suffix = crypto.randomUUID().replaceAll("-", "").slice(0, 8);
    const viewer = await insertAccountWithActor(tx, {
