...
This policy is adapted from [Fedify's AI policy][1], which is inspired by
[Ghostty's AI policy][2].

The Hackers' Pub project has the following rules for AI usage:
 ...
 -  *Pull requests created in any way by AI can only be for accepted issues or
    maintainer-approved work items.*  Drive-by pull requests that do not
    reference an accepted issue or maintainer approval will be closed.  If AI
    isn't disclosed but a maintainer suspects its use, the PR will be closed.
    If you want to share code for a non-accepted issue, open a discussion or
    attach it to an existing discussion.
...
These rules apply only to outside contributions to Hackers' Pub.  Maintainers
...
Please remember that Hackers' Pub is maintained by humans.
...
Hackers' Pub is written with AI assistance, and maintainers may use AI tools as
a productive tool in their workflow.  As a project, we welcome AI as a tool!
