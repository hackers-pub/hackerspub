...
export function QuotedNoteCard(props: QuotedNoteCardProps) {
  ...
  const [linkPreviewActive, setLinkPreviewActive] = createSignal(false);
        ...
        <div class={props.class} classList={props.classList}>
          ...
          >
            <InternalLink
              ...
              tabIndex={-1}
              class={cn(
                "block w-0 h-0 border-l-[15px] border-r-[15px] border-b-[20px] border-l-transparent border-r-transparent border-b-muted ml-4 transition-colors",
                linkPreviewActive() && "border-b-muted/80",
              )}
              onMouseEnter={() => setLinkPreviewActive(true)}
              onMouseLeave={() => setLinkPreviewActive(false)}
            />
          </Show>
          <Show
          ...
          >
            <div
              ...
              onMouseEnter={() => setLinkPreviewActive(true)}
              onMouseLeave={() => setLinkPreviewActive(false)}
              onFocusIn={() => setLinkPreviewActive(true)}
              onFocusOut={(event) => {
                const nextTarget = event.relatedTarget as Node | null;
                if (!nextTarget || !event.currentTarget.contains(nextTarget)) {
                  setLinkPreviewActive(false);
                }
              }}
            >
              <Show when={props.linkPreview}>
                <InternalLink
                  href={post.url ?? post.iri}
                  internalHref={getQuotedPostInternalHref(post)}
                  aria-label={t`Open post`}
                  class="absolute inset-0 z-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-muted"
                />
              </Show>
              <div
                class={cn(
