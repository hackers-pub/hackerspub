...
export function NewsStoryCard(props: NewsStoryCardProps) {
  ...
  const monogram = createMemo(() => host().charAt(0).toUpperCase() || "?");

  const sharesText = (count: number) =>
    i18n._(
      msg`${plural(count, { one: "# share", other: "# shares" })}`,
    );

  return (
    <Show keyed when={story()}>
      {(s) => (
        <article class="border-b px-4 py-4 transition-colors last:border-none hover:bg-muted/30">
          <div class="flex gap-4">
            <div class="hidden shrink-0 sm:block">
              <Show
                ...
                when={s.image}
                fallback={
                  <div class="flex size-16 items-center justify-center rounded-md border bg-muted/40 font-mono text-2xl text-muted-foreground/50 select-none">
                    {monogram()}
                  </div>
                }
              >
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="mt-1 truncate text-sm text-muted-foreground/70">
                <span class="font-medium">{host()}</span>
                <Show when={s.siteName && s.siteName !== host()}>
                  <span>· {s.siteName}</span>
                </Show>
              </p>
              <p class="mt-1.5 flex flex-wrap items-center gap-x-2 text-sm text-muted-foreground">
                <span>{sharesText(s.postCount)}</span>
