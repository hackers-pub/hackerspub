...
describe("onBlocked()", () => {
  ...
  it("rolls back the block when relationship cleanup cannot be enqueued", async () => {
    await withRollback(async (tx) => {
      ...
      const followIri = `https://blocking.example/follows/${crypto.randomUUID()}`;
      ...
      const block = new Block({
        id: new URL(`https://blocking.example/blocks/${crypto.randomUUID()}`),
...
describe("reconcileFollowAcceptance()", () => {
  it("uses IRI path first and skips fallback when it succeeds", async () => {
    ...
    const accepted = await reconcileFollowAcceptance(repo, {
  ...
  it("falls back to actor IDs when IRI path does not match", async () => {
    const calls = { byIri: 0, byActorIds: 0 };
    const repo: FollowAcceptanceRepository = {
      async acceptByIri() {
        calls.byIri++;
...
describe("reconcileFollowRejection()", () => {
  it("uses IRI path first and skips fallback when it succeeds", async () => {
    ...
    const rejected = await reconcileFollowRejection(repo, {
      followIri: "https://remote.example/follow/123",
      followerActorId: FOLLOWER_ACTOR_ID,
      followeeActorId: FOLLOWEE_ACTOR_ID,
    });
...
describe("reconcileFollowAcceptanceFromObjectId()", () => {
  it("accepts a pending outgoing follow using object IRI metadata", async () => {
    ...
    const accepted = await reconcileFollowAcceptanceFromObjectId(repo, {
      followIri: "https://hackers.pub/ap/actors/local#follow/1",
  ...
  it("ignores unmatched object IRIs", async () => {
    const calls = { findPending: 0, byIri: 0, byActorIds: 0 };
    const repo: FollowAcceptanceRepository & PendingOutgoingFollowRepository = {
      async findPendingOutgoingByIri() {
        calls.findPending++;
...
describe("reconcileFollowRejectionFromObjectId()", () => {
  it("rejects a pending outgoing follow using object IRI metadata", async () => {
    const calls = { findPending: 0, byIri: 0, byActorIds: 0 };
    ...
    const rejected = await reconcileFollowRejectionFromObjectId(repo, {
      followIri: "https://hackers.pub/ap/actors/local#follow/1",
      followeeIri: "https://remote.example/users/followee",
    });
    assert.deepEqual(rejected, true);
  ...
  it("ignores unmatched reject object IRIs", async () => {
    const calls = { findPending: 0, byIri: 0, byActorIds: 0 };
    const repo: FollowRejectionRepository & PendingOutgoingFollowRepository = {
      async findPendingOutgoingByIri() {
        calls.findPending++;
