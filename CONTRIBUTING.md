...
To run web-next, the new web frontend for Hackers' Pub, follow these steps:
 ...
 2. Install [watchman](https://facebook.github.io/watchman/docs/install). This is to run `relay-compiler` whenever files are changed.
 
 3. Run the development server with the command `VITE_API_URL=http://localhost:8000/graphql deno task dev`. The legacy server must be running at this point, as it also serves as the GraphQL API server for web-next.
 
 4. Access `localhost:3000` to see the new look of Hackers' Pub.
